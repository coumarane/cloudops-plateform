...
function StatusChip({ status }: { status: ManagedSecret["status"] }) {
  if (status === "Overdue") {
        ...
        <tbody>
          {secrets.map((secret) => (
            <tr key={secret.id} className="border-b border-outline last:border-b-0">
              ...
              <td className="p-3 font-mono text-xs text-muted">{secret.namespace}</td>
              ...
              <td className="p-3">
                <div className="flex flex-wrap justify-end gap-2">
                  {(["update", "rotate", "validate", "history"] as const).map((action) => (
                    <button
                      key={action}
                      type="button"
                      onClick={() => onAction(secret, action)}
                      className={
                        action === "rotate" && secret.environment === "PRD"
