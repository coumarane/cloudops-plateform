...
from app.services.alibaba_sync import run_health_scan
...
@celery_app.task(
    name="tasks.alibaba_cluster_health.scan_health",
    bind=True,
...
def scan_health(self, job_id: str, correlation_id: str) -> int:
    ...
    logger.info("Starting ACK health scan job_id=%s", job_id)
    try:
        return run_health_scan(job_id)
    except (AlibabaAuthError, AlibabaPermissionError):
        logger.exception("Non-retryable Alibaba error during ACK health scan")
        raise
