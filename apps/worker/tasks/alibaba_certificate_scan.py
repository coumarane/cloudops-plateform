...
from app.services.alibaba_sync import run_certificate_scan
...
@celery_app.task(
    name="tasks.alibaba_certificate_scan.scan_certificates",
    bind=True,
...
def scan_certificates(self, job_id: str, correlation_id: str) -> int:
    ...
    logger.info("Starting Alibaba certificate discovery job_id=%s", job_id)
    try:
        return run_certificate_scan(job_id)
    except (AlibabaAuthError, AlibabaPermissionError):
        logger.exception("Non-retryable Alibaba error during certificate discovery")
        raise
